// aux-leds.c: Aux LED functions for Tungsten LOW.
...
void rgb_led_update(uint8_t mode, uint16_t arg) {
    ...
    if (pattern == 3) {
        ...
        static const uint8_t animation[] = {2, 0};
    ...
    //ONLY update the button LED if it is enabled, otherwise update the RGB AUX LEDs
    #ifdef USE_BUTTON_LED
    button_led_set(button_led_result);
    //make an exception for flashing front AUX
    if (pattern == 3) {
        rgb_led_set(result);
    }
    #else
